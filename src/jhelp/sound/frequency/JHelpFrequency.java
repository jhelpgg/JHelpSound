...
public class JHelpFrequency
{
   ...
   void internalPlay()
   {
      synchronized(this.lock)
      {
         ...
         if((this.startPosition == 0) || (this.startPosition >= length))
         {
            ...
            for(final FrequencyInformation frequencyInformation : this.chanels)
            {
               ...
               {
                  this.byteArray.writeShortLittleEndian((short) ((frequencyInformation.volume1 * JHelpFrequency.MAX_VOLUME * 0.01 * Math.sin(angle1))));
                  this.byteArray.writeShortLittleEndian((short) ((frequencyInformation.volume2 * JHelpFrequency.MAX_VOLUME * 0.01 * Math.sin(angle2))));
      ...
      final AudioFormat audioFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, 44100 * 2, 16, 2, 4, 44100, false);
      ...
      catch(final Exception exception)
      {
         ...
         {
            Debug.printMark(DebugLevel.INFORMATION, mixerInfo.toString());
            final Mixer mixer = AudioSystem.getMixer(mixerInfo);

            for(final Line.Info lineInfo : mixer.getSourceLineInfo())
            {
               Debug.println(DebugLevel.INFORMATION, lineInfo.toString());
               Debug.println(DebugLevel.INFORMATION, "LineClass=", lineInfo.getLineClass());

               try
               {
                  final Line line = AudioSystem.getLine(lineInfo);
                  final SourceDataLine sourceDataLine = (SourceDataLine) line;
                  Debug.println(DebugLevel.INFORMATION, "sourceDataLine=", sourceDataLine);
   ...
   public void addFrequency(int frequencyChanel1, int volume1, int frequencyChanel2, int volume2)
   {
      ...
      frequencyChanel1 = Math.max(JHelpFrequency.MIN_FREQUENCY, Math.min(JHelpFrequency.MAX_FREQUENCY, frequencyChanel1));
      frequencyChanel2 = Math.max(JHelpFrequency.MIN_FREQUENCY, Math.min(JHelpFrequency.MAX_FREQUENCY, frequencyChanel2));
